...
export const exportToCSV = (data, filename = 'export', headers = null) => {
    if (!data || data.length === 0) {
        // No usar alert aquí, el componente que llama debe manejar el error
...
/**
 * Exporta rutina a PDF desde el backend
 * @param {Object} routine - Objeto de rutina con routine_id
 */
export const exportRoutineToPDF = async (routine) => {
    if (!routine || !routine.routine_id) {
        throw new Error('La rutina no tiene un ID válido para exportar');
    }

    try {
        // Importar api dinámicamente para evitar dependencias circulares si es necesario
        // En la mayoría de casos, podemos importarlo estáticamente
        const apiModule = await import('../services/api');
        const api = apiModule.default;
        
        const response = await api.get(`/routines/${routine.routine_id}/export/pdf`, {
            responseType: 'blob'
        });
        ...
        let filename = `rutina_${routine.name?.replace(/[^a-z0-9]/gi, '_') || 'rutina'}_${new Date().toISOString().split('T')[0]}.pdf`;
        ...
        console.error('Error al exportar rutina a PDF:', error);
        throw new Error(error.response?.data?.error || 'Error al exportar la rutina a PDF');
    }
};
...
export const exportRoutine = (routine) => {
    if (!routine || !routine.exercises || routine.exercises.length === 0) {
        throw new Error('La rutina no tiene ejercicios para exportar');
    }

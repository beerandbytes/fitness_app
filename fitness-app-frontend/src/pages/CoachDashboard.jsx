...
import { Users, AlertCircle, TrendingUp, Target, Calendar, UserPlus, FileText, Dumbbell, Utensils, Eye, Sparkles } from 'lucide-react';
import useToastStore from '../stores/useToastStore';
import logger from '@/utils/logger';
import { CoachDashboardSkeleton } from '../components/CoachDashboardSkeleton';
...
const CoachDashboard = () => {
    ...
    const [filter, setFilter] = useState('all'); // 'all', 'active', 'inactive' (mantener para compatibilidad)
    ...
    const filteredClients = useMemo(() => {
        ...
        if (filters) {
            ...
            const now = new Date();
            if (filters.lastActivity === 'today') {
                result = result.filter(c => c.daysSinceActivity === 0);
            } else if (filters.lastActivity === 'week') {
                result = result.filter(c => c.daysSinceActivity <= 7);
            } else if (filters.lastActivity === 'month') {
                result = result.filter(c => c.daysSinceActivity <= 30);
            }
            ...
            // Filtro simple (compatibilidad con código existente)
            if (filter === 'active') {
                result = result.filter(c => !c.needsAttention);
        ...
        return result;
    }, [clients, searchTerm, filters, filter]);
    ...
    return (
        <AppLayout>
            <PageContainer>
                ...
                {/* Quick Actions Card - Rutinas Predefinidas */}
                <div className="mb-8 backdrop-blur-xl bg-gradient-to-br from-primary-50 to-primary-100 dark:from-primary-900/20 dark:to-primary-800/20 rounded-3xl p-6 border-2 border-primary-200/50 dark:border-primary-800/50 shadow-lg">
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                        <div className="flex items-start gap-4">
                            <div className="w-14 h-14 backdrop-blur-sm bg-primary-500/20 dark:bg-primary-500/30 rounded-2xl flex items-center justify-center border border-primary-300/50 dark:border-primary-700/50">
                                <Sparkles className="w-7 h-7 text-primary-600 dark:text-primary-400" />
                            </div>
                            <div>
                                <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-1">
                                    Rutinas Predefinidas Disponibles
                                </h3>
                                <p className="text-sm text-gray-600 dark:text-gray-400">
                                    Explora y duplica rutinas profesionales listas para usar. Perfectas para inspirarte o asignar directamente a tus clientes.
                                </p>
                            </div>
                        </div>
                        <button
                            onClick={() => navigate('/coach/templates', { state: { tab: 'predefined' } })}
                            className="flex items-center gap-2 px-6 py-3 bg-primary-600 dark:bg-primary-500 text-white rounded-xl font-medium hover:bg-primary-700 dark:hover:bg-primary-600 transition-all shadow-lg hover:shadow-xl active:scale-95 whitespace-nowrap"
                        >
                ...
                {clients.length === 0 && !loading && (
                    <EmptyState
                        icon={<Users className="w-16 h-16 text-gray-400 dark:text-gray-600" />}
                        title="Aún no tienes clientes"
                        description="Comienza invitando a tus primeros clientes para gestionar su progreso y ayudarlos a alcanzar sus objetivos."
                        actionLabel="Invitar Primer Cliente"
                        actionOnClick={() => setInviteModalOpen(true)}
                    />
                )}
                ...
                {/* Content */}
                {viewMode === 'carousel' ? (
                    <div className="overflow-x-auto pb-4 -mx-4 sm:mx-0 px-4 sm:px-0 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600 scrollbar-track-transparent">
...
const ClientCard = ({ client, onClick, navigate, onAssignRoutine, onAssignDiet }) => {
    ...
    return (
        <div
            className={`w-[280px] sm:w-[320px] min-w-[280px] sm:min-w-[320px] backdrop-blur-xl bg-white/60 dark:bg-black/60 rounded-3xl p-4 sm:p-6 border-2 transition-all duration-500 hover:shadow-xl hover:scale-[1.02] ${

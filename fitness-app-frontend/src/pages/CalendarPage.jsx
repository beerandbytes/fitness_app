...
const CalendarPage = () => {
    ...
    // Obtener detalles de una rutina seleccionada
    const fetchRoutineDetails = useCallback(async (routineId) => {
        if (!routineId) {
        ...
        try {
            ...
            setSelectedRoutineDetails(response.data.routine);
        } catch (error) {
    ...
    const handleScheduleRoutine = async () => {
        if (!selectedDate || !selectedRoutine) {
            toast.warning('Por favor selecciona una fecha y una rutina');
            return;
        }
        ...
        try {
            ...
            const routineName = routines.find(r => r.routine_id === parseInt(selectedRoutine))?.name || 'Rutina';
            toast.success(`Rutina "${routineName}" programada para ${format(selectedDate, 'd MMMM', { locale: es })}`);
    ...
    const handleToggleComplete = async (scheduledId, currentStatus) => {
        try {
            ...
            toast.success(!currentStatus ? 'Rutina marcada como completada' : 'Rutina marcada como no completada');
            ...
            toast.error(error.response?.data?.error || 'Error al actualizar el estado de la rutina');
        }
    };

    // Eliminar rutina planificada
    const handleDeleteClick = (scheduledId, routineName) => {
        setDeleteConfirm({ open: true, scheduledId, routineName });
    };

    const handleDeleteConfirm = async () => {
        const { scheduledId, routineName } = deleteConfirm;
        setDeleting(true);
        try {
            await api.delete(`/calendar/schedule/${scheduledId}`);
            toast.success(`Rutina "${routineName}" eliminada del calendario`);
            setDeleteConfirm({ open: false, scheduledId: null, routineName: '' });
            fetchScheduledRoutines();
        } catch (error) {
            logger.error('Error al eliminar rutina planificada:', error);
            toast.error(error.response?.data?.error || 'Error al eliminar la rutina planificada');
        } finally {
            setDeleting(false);
        }
    };
    ...
    return (
        <AppLayout>
                                        ...
                                        {isCurrentMonth && dayRoutines.length > 0 && (
                                            <div className="space-y-1">
                                                {dayRoutines.slice(0, 2).map((sr) => (
                                                    <Link
                                                        key={sr.scheduled_id}
                                                        to={`/routines/${sr.routine_id}`}
                                                        className={`text-xs p-1 rounded truncate block hover:underline ${
                                                            sr.is_completed
                                                                ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 hover:text-green-800 dark:hover:text-green-300'
                                                                : 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300'
                                                        }`}
                                    ...
                                    >
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button
                                                    onClick={() => handleDeleteClick(sr.scheduled_id, sr.routine_name)}
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        fetchRoutineDetails(e.target.value);
                                    }}
                                >
                                        ...
                                        <>
                                            <div className="flex items-start justify-between mb-2">
                                                <div className="flex-1">
                                                    ...
                                                    {selectedRoutineDetails.description && (
                                                    ...
                                                    <p className="text-xs text-gray-500 dark:text-gray-500">
                                                        ...
                                                        {selectedRoutineDetails.exercises?.length === 1 ? 'ejercicio' : 'ejercicios'}
                                                    </p>
                                                </div>
                                                <Link
                                                    to={`/routines/${selectedRoutine}`}
                                                    target="_blank"
                                                    className="ml-3 text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 text-sm font-medium flex items-center gap-1"
                                                >
                                                    Ver detalles
            ...
            <ConfirmDialog
                ...
                title="¿Eliminar rutina del calendario?"
                description={`¿Estás seguro de eliminar "${deleteConfirm.routineName}" de esta fecha?`}

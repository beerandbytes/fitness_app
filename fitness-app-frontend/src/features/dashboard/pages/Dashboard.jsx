...
const Dashboard = () => {
    ...
    // Manejar cuando se completa una tarea
    const handleTaskComplete = useCallback(async (task) => {
        // Recargar los datos del dashboard cuando se complete una tarea
        await Promise.all([fetchDailyLog(), fetchGoal()]);
    }, [fetchDailyLog, fetchGoal]);
                ...
                {!isToday(currentDate) && (
                    <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl">
                        <p className="text-sm text-blue-800 dark:text-blue-300">
                            📊 Comparando con {format(subDays(currentDate, 1), 'd MMMM', { locale: es })}
                        </p>
                    </div>
                )}

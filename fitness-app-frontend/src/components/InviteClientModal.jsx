...
const InviteClientModal = ({ open, onOpenChange }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await api.post('/coach/invite', { email });
            setSuccess(true);
            setEmail('');
    ...
    return (
        <Dialog.Root open={open} onOpenChange={onOpenChange}>
            <Dialog.Portal>
                ...
                <Dialog.Content 
                    ref={contentRef}
                    className="fixed top-0 md:top-1/2 left-0 md:left-1/2 md:transform md:-translate-x-1/2 md:-translate-y-1/2 bg-white dark:bg-gray-900 rounded-t-2xl md:rounded-2xl shadow-xl p-6 w-full md:w-[calc(100%-2rem)] md:mx-4 max-w-md max-h-[90vh] md:max-h-[90vh] h-[90vh] md:h-auto overflow-y-auto z-50 border border-gray-200 dark:border-gray-800 focus:outline-none"
                    onEscapeKeyDown={(e) => {
                        if (loading) {
                            e.preventDefault();
                        }

...
const ExerciseSearchAndAdd = ({ onLogUpdated, onExerciseSelect, selectedExercise, onExerciseAddedOptimistically }) => {
    ...
    const rateLimitedSearch = useRateLimit(async (query) => {
        ...
        try {
            ...
            const exercisesWithThumbnails = filteredExercises.map(exercise => {
                ...
                const gifUrl = exercise.gif_url && !exercise.gif_url.toLowerCase().includes('giphy') ? exercise.gif_url : null;
                const videoUrl = exercise.video_url && !exercise.video_url.toLowerCase().includes('giphy') ? exercise.video_url : null;
                return {
                    ...exercise,
                    gif_url: gifUrl,
                    video_url: videoUrl,
                    thumbnail: gifUrl || videoUrl || null
                };
            });
    ...
    const loadExerciseMedia = useCallback(async (exerciseName) => {
        ...
        try {
            ...
            if (response.data) {
                // Filtrar URLs de Giphy
                const gifUrl = response.data.gif_url;
                const videoUrl = response.data.video_url;
                
                // Rechazar URLs de Giphy
    ...
    const handleSubmit = async (e) => {
        ...
        // Crear ejercicio optimista para mostrar inmediatamente
        const optimisticExercise = {
            daily_exercise_id: `temp-${Date.now()}`,
        ...
        // Optimistic update: mostrar ejercicio inmediatamente
        if (onExerciseAddedOptimistically) {
            onExerciseAddedOptimistically(optimisticExercise);
        }

...
import { useOptimisticUpdate, createValueOptimisticUpdate } from '../hooks/useOptimisticUpdate';
...
const WeightForm = ({ currentDate, currentWeight, onLogUpdated }) => {
    ...
    const { execute: executeOptimistic, isPending } = useOptimisticUpdate({
        successMessage: `Peso registrado correctamente`,
        errorMessage: 'Error al registrar el peso',
    });
    ...
    const onSubmit = async (data) => {
        const newWeight = data.weight.toFixed(2);
        const newLog = {
        ...
        const optimisticUpdate = createValueOptimisticUpdate(
            (value) => {
                if (onLogUpdated) {
                    onLogUpdated(value);
                }
        ...
        try {
            ...
            // El error ya se maneja en useOptimisticUpdate
            throw error;
        }
    };

...
const exercises = pgTable('exercises', {
    ...
    name: varchar('name', { length: 100 }).unique().notNull(),
    name_es: varchar('name_es', { length: 100 }), // Nombre en español (equivalencia en castellano)
    ...
    description: text('description'), // Descripción del ejercicio (cómo realizarlo)
    muscles: text('muscles'), // Músculos trabajados (JSON array o texto separado por comas)
    equipment: text('equipment'), // Equipamiento necesario (JSON array o texto separado por comas)
...
// =================================================================
// 28. Definición de la tabla User_Follows (Sistema Social - Seguir usuarios)
// =================================================================
const userFollows = pgTable('user_follows', {
    follow_id: serial('follow_id').primaryKey(),
    follower_id: integer('follower_id').references(() => users.user_id).notNull(), // Usuario que sigue
    following_id: integer('following_id').references(() => users.user_id).notNull(), // Usuario seguido
    created_at: timestamp('created_at').defaultNow(),
}, (table) => {
    return {
        followUnique: unique('follow_unique').on(table.follower_id, table.following_id),
    };
});

// =================================================================
// 29. Definición de la tabla Shared_Workouts (Rutinas Compartidas)
// =================================================================
const sharedWorkouts = pgTable('shared_workouts', {
    share_id: serial('share_id').primaryKey(),
    user_id: integer('user_id').references(() => users.user_id).notNull(), // Usuario que comparte
    routine_id: integer('routine_id').references(() => routines.routine_id).notNull(),
    is_public: boolean('is_public').default(true).notNull(), // Si es público o solo para seguidores
...
// =================================================================
// 30. Definición de la tabla Workout_Likes (Likes en rutinas compartidas)
// =================================================================
const workoutLikes = pgTable('workout_likes', {
    like_id: serial('like_id').primaryKey(),
    user_id: integer('user_id').references(() => users.user_id).notNull(),
    share_id: integer('share_id').references(() => sharedWorkouts.share_id).notNull(),
    created_at: timestamp('created_at').defaultNow(),
}, (table) => {
    return {
        likeUnique: unique('like_unique').on(table.user_id, table.share_id),
    };
});

// =================================================================
// 31. Definición de la tabla Workout_Comments (Comentarios en rutinas compartidas)
// =================================================================
const workoutComments = pgTable('workout_comments', {
...
// =================================================================
// 32. Definición de la tabla Body_Measurements (Medidas Corporales)
// =================================================================
const bodyMeasurements = pgTable('body_measurements', {
    measurement_id: serial('measurement_id').primaryKey(),
    user_id: integer('user_id').references(() => users.user_id).notNull(),
    date: date('date').notNull(),
    chest: numeric('chest'), // Pecho en cm
    waist: numeric('waist'), // Cintura en cm
    hips: numeric('hips'), // Cadera en cm
    arms: numeric('arms'), // Brazo en cm
    thighs: numeric('thighs'), // Muslo en cm
    neck: numeric('neck'), // Cuello en cm
    shoulders: numeric('shoulders'), // Hombros en cm
    body_fat_percentage: numeric('body_fat_percentage'), // Porcentaje de grasa corporal
    created_at: timestamp('created_at').defaultNow(),
    updated_at: timestamp('updated_at').defaultNow(),
}, (table) => {
    return {
        measurementUnique: unique('measurement_unique').on(table.user_id, table.date),
    };
});

// =================================================================
// 33. Definición de la tabla Progress_Photos (Fotos de Progreso)
// =================================================================
const progressPhotos = pgTable('progress_photos', {
    photo_id: serial('photo_id').primaryKey(),
    user_id: integer('user_id').references(() => users.user_id).notNull(),
    date: date('date').notNull(),
    photo_front: varchar('photo_front', { length: 500 }), // URL de foto frontal
    photo_side: varchar('photo_side', { length: 500 }), // URL de foto lateral
    photo_back: varchar('photo_back', { length: 500 }), // URL de foto trasera
    weight: numeric('weight'), // Peso al momento de la foto
...
module.exports = {
    ...
    userFollows,     // <-- NUEVO: Sistema social - seguir usuarios
    sharedWorkouts,  // <-- NUEVO: Rutinas compartidas públicamente
    workoutLikes,    // <-- NUEVO: Likes en rutinas compartidas
    workoutComments, // <-- NUEVO: Comentarios en rutinas compartidas
    bodyMeasurements, // <-- NUEVO: Medidas corporales
    progressPhotos,   // <-- NUEVO: Fotos de progreso
};
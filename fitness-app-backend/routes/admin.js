...
// ---------------------------------------------------------------------------
// 11) Obtener métricas del sistema completo
...
router.get('/metrics', async (req, res) => {
    try {
        ...
        // 2. Estadísticas de actividad (ejercicios y comidas)
        const periodStartStr = periodStart.toISOString().split('T')[0];
        const todayStr = now.toISOString().split('T')[0];

        const dailyLogsInPeriod = await db
            .select({
                date: dailyLogs.date,
                user_id: dailyLogs.user_id,
                consumed_calories: dailyLogs.consumed_calories,
                burned_calories: dailyLogs.burned_calories,
            })
            .from(dailyLogs)
            .where(and(gte(dailyLogs.date, periodStartStr), lte(dailyLogs.date, todayStr)))
            .orderBy(asc(dailyLogs.date));

        const exerciseLogsInPeriod = await db
            .select({
                date: dailyLogs.date,
                burned_calories: dailyExercises.burned_calories,
            })
        ...
        const mealLogsInPeriod = await db
            .select({
                date: dailyLogs.date,
            })
            .from(mealItems)
        ...
        const uniqueExerciseDays = new Set(exerciseLogsInPeriod.map(log => log.date)).size;
        const uniqueMealDays = new Set(mealLogsInPeriod.map(log => log.date)).size;
        const totalCaloriesBurned = exerciseLogsInPeriod.reduce((sum, log) => sum + parseFloat(log.burned_calories || 0), 0);
        const totalCaloriesConsumed = dailyLogsInPeriod.reduce((sum, log) => sum + parseFloat(log.consumed_calories || 0), 0);

        // 3. Estadísticas de rutinas
        const totalRoutines = await db
            .select({ count: sql`count(*)` })
            .from(routines);
        const routineCount = parseInt(totalRoutines[0]?.count || 0);

        const activeRoutines = await db
            .select({ count: sql`count(*)` })
            .from(routines)
            .where(eq(routines.is_active, true));
        const activeRoutineCount = parseInt(activeRoutines[0]?.count || 0);

        // 4. Estadísticas de ejercicios disponibles
        const totalExercises = await db
            .select({ count: sql`count(*)` })
            .from(exercises);
        const exerciseCount = parseInt(totalExercises[0]?.count || 0);

        // 5. Estadísticas de alimentos
        const totalFoods = await db
            .select({ count: sql`count(*)` })
            .from(foods);
        const foodCount = parseInt(totalFoods[0]?.count || 0);

        // 6. Crecimiento de usuarios por día (para gráfico)
        ...
        // 7. Actividad diaria (ejercicios y comidas)
        const activityByDate = {};
        dailyLogsInPeriod.forEach(log => {
            ...
            activityByDate[dateStr].caloriesConsumed += parseFloat(log.consumed_calories || 0);
            activityByDate[dateStr].caloriesBurned += parseFloat(log.burned_calories || 0);
        });

        exerciseLogsInPeriod.forEach(log => {
            const dateStr = log.date;
            if (activityByDate[dateStr]) {
                activityByDate[dateStr].exercises += 1;
            }
        });

        mealLogsInPeriod.forEach(log => {
            const dateStr = log.date;
            if (activityByDate[dateStr]) {
                activityByDate[dateStr].meals += 1;
            }
        });

        const activityData = Object.values(activityByDate).sort((a, b) => a.date.localeCompare(b.date));

        return res.status(200).json({
            message: 'Métricas del sistema cargadas correctamente.',
            period,
            ...
            users: {
                ...
                admins,
                completedOnboarding,
                onboardingCompletionRate: allUsers.length > 0 
                    ? ((completedOnboarding / allUsers.length) * 100).toFixed(1) 
                    : 0,
            },
            activity: {
                uniqueExerciseDays,
                uniqueMealDays,
                totalCaloriesBurned: totalCaloriesBurned.toFixed(0),
                totalCaloriesConsumed: totalCaloriesConsumed.toFixed(0),
                averageCaloriesBurnedPerDay: uniqueExerciseDays > 0 
                    ? (totalCaloriesBurned / uniqueExerciseDays).toFixed(0) 
                    : 0,
                averageCaloriesConsumedPerDay: uniqueMealDays > 0 
                    ? (totalCaloriesConsumed / uniqueMealDays).toFixed(0) 
                    : 0,
            },
            routines: {
                total: routineCount,
                active: activeRoutineCount,
                inactive: routineCount - activeRoutineCount,
            },
            exercises: {
                total: exerciseCount,
            },
        ...
        logger.error('Error al obtener métricas del sistema (admin):', { error: error.message, stack: error.stack });
        return res.status(500).json({ error: 'Error interno del servidor al obtener métricas.' });
    }
});

// ---------------------------------------------------------------------------
// 12) Generar rutina automática de 5 días para un usuario

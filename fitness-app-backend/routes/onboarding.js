...
// GET /api/onboarding/stats - Obtener estadísticas públicas para social proof
router.get('/stats', async (req, res) => {
    try {
        ...
        const userCount = parseInt(totalUsers[0]?.count || 0);
        ...
        const activeUsers = await db
            .select({ 
                count: sql`count(distinct ${dailyLogs.user_id})` 
            })
            .from(dailyLogs)
            .where(sql`${dailyLogs.date} >= ${dateStr}`);
        ...
        const completedOnboarding = await db
            .select({ count: sql`count(*)` })
            .from(users)
            .where(eq(users.onboarding_completed, true));
        const completedCount = parseInt(completedOnboarding[0]?.count || 0);

        // Calcular porcentaje de completación (para mostrar)
        const completionRate = userCount > 0 
            ? Math.round((completedCount / userCount) * 100) 
            : 0;

        return res.status(200).json({
            totalUsers: userCount || 10000, // Default para mostrar si no hay datos
            activeUsers: activeCount || 5000,
            usersAchievingGoals: Math.round((activeCount || 5000) * 0.7), // Estimado 70%
            completionRate: completionRate || 85,
        });

    } catch (error) {
        logger.error('Error al obtener estadísticas de onboarding:', { 
            error: error.message, 
            stack: error.stack 
        });

...
function isAdminEmail(email) {
    ...
    // Logging para debugging (solo en desarrollo o si está habilitado)
    if (process.env.NODE_ENV !== 'production' || process.env.DEBUG_ADMIN === 'true') {
        logger.debug('Verificando admin email:', {
            email,
            adminEmailsRaw: raw ? `${raw.substring(0, 50)}...` : 'NO CONFIGURADA',
    ...
    if (!raw || !email) {
        if (process.env.NODE_ENV !== 'production' || process.env.DEBUG_ADMIN === 'true') {
            logger.debug('isAdminEmail retorna false:', {
                reason: !raw ? 'ADMIN_EMAILS no configurada' : 'email vacío',
                email
            });
        }
        return false;
    ...
    // Normalizar y limpiar la lista de emails
    const list = raw
        .split(',')
        .map((e) => e.trim().toLowerCase())
        .filter(Boolean)
        .filter(e => e.includes('@')); // Validar que tenga formato de email básico
    
    const emailLower = email.toLowerCase().trim();
    const isAdmin = list.includes(emailLower);
    
    if (isAdmin && (process.env.NODE_ENV !== 'production' || process.env.DEBUG_ADMIN === 'true')) {
        logger.info(`Email reconocido como admin: ${email}`);
    }
...
router.post('/register', 
    ...
    async (req, res) => {
        ...
        try {
            ...
            // CORREGIDO: Usar un secret diferente para refresh tokens (mejor seguridad)
            // Si JWT_REFRESH_SECRET no está configurado, generar uno basado en JWT_SECRET
...
router.post('/refresh', async (req, res) => {
    ...
    try {
        // CORREGIDO: Usar el mismo secret que se usó para firmar (consistencia)
        const refreshSecret = process.env.JWT_REFRESH_SECRET || `${process.env.JWT_SECRET}_refresh`;
        ...
        // CORREGIDO: Usar el mismo secret que se usó para verificar

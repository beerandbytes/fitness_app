...
// Obtener instancia de Socket.IO si está disponible
let io = null;
try {
    const socketServer = require('../socketServer');
    // El io se pasa desde index.js después de inicializar
} catch (e) {
    // Socket.IO no está disponible, continuar sin él
}

// Función para obtener io desde el módulo principal
const getIO = () => {
...
router.post('/', 
    ...
    asyncHandler(async (req, res) => {
        ...
        // Emitir mensaje a través de WebSocket si está disponible
        const socketIO = getIO();
        if (socketIO) {
            const roomId = [senderId, receiver_id].sort().join('_');
            socketIO.to(roomId).emit('new_message', newMessage[0]);
        }

...
const routeValidations = {
  ...
  createExercise: [
    ...
    body('name_es')
      ...
      .trim()
      .isLength({ max: 100 })
      .withMessage('name_es no puede tener más de 100 caracteres'),
    body('category')
    ...
    body('description')
      ...
      .trim()
      .isLength({ max: 2000 })
      .withMessage('description no puede tener más de 2000 caracteres'),
    // Validar base64 para imágenes/videos subidos
    body('image_base64')
      .optional()
      .custom((value) => {
        if (!value) return true;
        if (typeof value !== 'string') return false;
        // Verificar que sea base64 válido y que tenga un tipo MIME de imagen
        const base64Pattern = /^data:(image\/(jpeg|jpg|png|gif|webp)|video\/(mp4|webm|mov));base64,/;
        return base64Pattern.test(value);
      })
      .withMessage('image_base64 debe ser una imagen (jpeg, jpg, png, gif, webp) o video (mp4, webm, mov) en formato base64'),
    body('video_base64')
      .optional()
      .custom((value) => {
        if (!value) return true;
        if (typeof value !== 'string') return false;
        // Verificar que sea base64 válido y que tenga un tipo MIME de video
        const base64Pattern = /^data:video\/(mp4|webm|mov);base64,/;
        return base64Pattern.test(value);
      })
      .withMessage('video_base64 debe ser un video (mp4, webm, mov) en formato base64'),
  ],
